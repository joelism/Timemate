...
<body>
  ...
  <script>setTimeout(function(){ window.print(); }, 250);</script>
    ...
    function form(editId){
      ...
      // Auto-Standort aus Kontakte → Hauptadresse (nur wenn leer)
      let __locTouched = false;
      function applyCatMainAddress(){
        try{
          const cat = selCat && selCat.value;
          const addr = (catAddr && catAddr[cat]) ? catAddr[cat] : '';
          if(!addr) return;
          const locEl = byId('location') || document.querySelector('#dyn #location');
          if(!locEl) return;
          // mark user interaction
          if(locEl.tagName === 'INPUT'){
            if(!__locTouched && (!locEl.value || !locEl.value.trim())){
              locEl.value = addr;
            }
            if(!locEl.__bound){ locEl.addEventListener('input', ()=>{ __locTouched = true; }); locEl.__bound = true; }
            ...
            if(!locEl.__bound){ locEl.addEventListener('change', ()=>{ __locTouched = true; }); locEl.__bound = true; }
          }
        }catch(_){}
      }
...
// Notizen + Anhänge
      ...
      if(editing){
        ...
        if(byId('location') && !byId('location').value){ applyCatMainAddress(); }
      }
