...
/* ===========================
   TimeMate Enhancer — Standort-Vorschläge + Auto-Default + Personen-Adresse
   (ganz ans Ende deiner bestehenden app.js einfügen)
   =========================== */
(function(){
  if (window.__TM_ADDR_ENHANCER__) return;
  window.__TM_ADDR_ENHANCER__ = true;

  // ---- State lesen (egal welcher Key genutzt wird)
  ...
  function addressForPerson(name, cat){
    ...
    const { data } = readState();
    const arr = Array.isArray(data.contacts) ? data.contacts : [];
    // zuerst innerhalb der Kategorie
    let c = arr.find(c=>{
  ...
  function findLocationInput(root){
    ...
    for (const inp of inputs){
      ...
      if (lbl && /(standort|adresse)/i.test(lbl.textContent||'')) return inp; // erkennt dein „Standort“-Feld
    }
    return null;
  }
  ...
  function applyLocationLogic(form){
    ...
    if (personField) personField.addEventListener('change', ()=>{
      const cat = catSel.value || 'Unkategorisiert';
      const pAddr = addressForPerson(currentPersonName(), cat);
      if (pAddr) locInput.value = pAddr; // sofort übernehmen
      rebuild();
    });
    if (personField) personField.addEventListener('input', rebuild);

    // Beim Speichern: leer -> Hauptadresse; „Andere…“ -> leeren
  ...
  // ---- Automatisch anwenden, sobald das Termin-Formular im DOM ist
