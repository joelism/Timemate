(function () {
  ...
  function init() {
    ...
    // Tabs anpassen
    (function adjustTabs(){
      // Tab-Leiste scrollfähig machen (Fallback, falls CSS fehlt)
      try{ const nav=document.querySelector('.tabs'); if(nav){ nav.style.overflowX='auto'; nav.style.webkitOverflowScrolling='touch'; nav.style.display='flex'; nav.style.gap='8px'; } }catch(_){ }
      ...
      // Aufgaben-Tab hinzufügen falls fehlend
    ...
    function form(editId){
      ...
      if(editing && editing.type!=='Aufgabe'){ const pdf=el('button',{type:'button'},'🧾 Terminbestätigung (PDF)'); pdf.onclick=()=>openConfirmationPDF(editing); s.append(pdf); setTimeout(()=>openConfirmationPDF(editing),150); }
      v.append(s);
    ...
    function renderItem(a, refresh){
      ...
      const b3=el('button',{type:'button'},'✏️ Bearbeiten'); b3.onclick=()=>route('new', a.id);
      if(a.type!=='Aufgabe'){ const b4=el('button',{type:'button'},'🧾 Bestätigung'); b4.onclick=()=>openConfirmationPDF(a); row.append(b1,b2,b3,b4);} else { row.append(b1,b2,b3); }
      it.append(row);
      return it;
    }
