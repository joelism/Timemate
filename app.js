...
<body>
  ...
  <script>setTimeout(function(){ window.print(); }, 250);</script>
    ...
    function form(editId){
      ...
      // Auto-Standort per Hauptadresse
      ;(function(){
        const MAIN_KEY='tmjw_mainAddresses';
        const mainAddrs = (()=>{ try{ return JSON.parse(localStorage.getItem(MAIN_KEY)||'{}')||{}; }catch(_){ return {}; }})();
        const locInput = byId('location') || document.querySelector('input#loc') || document.querySelector('input[id="loc"],input#location,input[name="location"]');
        selCat.addEventListener('change',()=>{
          try{
            if(!locInput) return;
            if(locInput.value && locInput.value.trim()!=='') return; // nicht überschreiben
            const cat = selCat.value;
            const addr = mainAddrs && cat ? mainAddrs[cat] : '';
            if(addr) locInput.value = addr;
          }catch(_){}
        });
      })();
      ...
      // Vorbelegung Standort, falls leer und Hauptadresse existiert
      (function(){
        try{
          const MAIN_KEY='tmjw_mainAddresses';
          const mainAddrs = JSON.parse(localStorage.getItem(MAIN_KEY)||'{}')||{};
          const locInput = byId('location') || document.querySelector('input#loc') || document.querySelector('input[id="loc"],input#location,input[name="location"]');
          const cat = selCat && selCat.value;
          if(locInput && (!locInput.value || !locInput.value.trim()) && mainAddrs && cat && mainAddrs[cat]){
            locInput.value = mainAddrs[cat];
          }
        }catch(_){}
      })();
    // Datum/Uhrzeit
    ...
    function settings(){
      v.innerHTML=`<section><h2>Einstellungen</h2>
        ...
        <h3>Hauptadressen pro Kategorie</h3>
        <div id="cat-mainaddr"></div>
        <div class="btnrow">
          <input id="cat-new-name" type="text" placeholder="Neue Kategorie">
          <input id="cat-new-addr" type="text" placeholder="Adresse">
          <button id="cat-add" type="button">Kategorie hinzufügen</button>
        </div>
    </div>
      ...
      // Hauptadressen: Laden, Rendern, Speichern
      const MAIN_KEY='tmjw_mainAddresses';
      const mainAddrs = (()=>{ try{ return JSON.parse(localStorage.getItem(MAIN_KEY)||'{}')||{}; }catch(_){ return {}; }})();

      function knownCategories(){
        const set=new Set();
        (state.items||[]).forEach(it=>{ if(it && it.category) set.add(String(it.category)); });
        return Array.from(set).sort((a,b)=>a.localeCompare(b,'de'));
      }

      function renderMainAddrs(){
        const host = byId('cat-mainaddr');
        if(!host) return;
        host.innerHTML='';
        const cats = knownCategories();
        if(cats.length===0){
          host.appendChild(document.createTextNode('Noch keine Kategorien vorhanden.'));
          return;
        }
        cats.forEach(cat=>{
          const row=document.createElement('div');
          row.className='row';
          const lab=document.createElement('label'); lab.className='half'; lab.textContent=cat;
          const inp=document.createElement('input'); inp.className='half'; inp.type='text'; inp.value=mainAddrs[cat]||''; inp.placeholder='Hauptadresse für ' + cat;
          inp.addEventListener('change',()=>{
            const val=inp.value.trim();
            if(val) mainAddrs[cat]=val; else delete mainAddrs[cat];
            localStorage.setItem(MAIN_KEY, JSON.stringify(mainAddrs));
          });
      ...
      // Manuell neue Kategorie hinzufügen
      const addBtn = byId('cat-add');
      if(addBtn){
        addBtn.onclick=()=>{
          const name=(byId('cat-new-name').value||'').trim();
          const addr=(byId('cat-new-addr').value||'').trim();
          if(!name){ alert('Bitte Kategorienamen eingeben.'); return; }
          const store=JSON.parse(localStorage.getItem(MAIN_KEY)||'{}')||{};
          if(addr) store[name]=addr; else delete store[name];
          localStorage.setItem(MAIN_KEY, JSON.stringify(store));
          byId('cat-new-name').value=''; byId('cat-new-addr').value='';
          renderMainAddrs();
        };
      }
