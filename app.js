...
<head>
<meta charset="utf-8">
<title>Terminbestätigung – ${esc(item.title||'(ohne Titel)')}</title>
<style>
  @page { margin: 18mm; }
  ...
  .badge { display:inline-block; border:1px solid #e5e7eb; border-radius:999px; padding:4px 10px; font-size:12px; }
...
<body>
  ...
  <div class="box">
    <div class="row"><div class="label">Titel</div><div><strong>${esc(item.title||'(ohne Titel)')}</strong></div></div>
    ...
    <div class="row"><div class="label">Anhänge</div><div>${
      (item.attachments&&item.attachments.length)
        ? esc(item.attachments.map(a=>a.name).join(', ')) : '—'
    }</div></div>
  ...
  <footer>
    ...
    <div class="brand">automatisch generiert durch TimeMate by J.w.</div>
  </footer>

  <script>
    // Auto-Print (der Nutzer kann als PDF speichern)
    setTimeout(function(){ window.print(); }, 200);
        ...
        if(!ww){ alert('Popup blockiert – bitte Popups für diese Seite erlauben.'); return; }
        ww.document.open('text/html');
        ww.document.write(html);
        ww.document.close();
      }catch(e){
        console.error('Fehler beim Erstellen der Bestätigung', e);
