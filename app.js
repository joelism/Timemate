...
function ov(){
  ...
  (function(){
    ...
    const todayTerms = (state.items||[]).filter(a=>a.status!=='archived' && a.type!=='Aufgabe' && isSameDay(new Date(a.datetime), now));
    const todayTasks = (state.items||[]).filter(a=>a.status!=='archived' && a.type==='Aufgabe' && isSameDay(new Date(a.datetime), now));
    const listAll = includeTasks==='on' ? [...todayTerms, ...todayTasks] : todayTerms;
    listAll.sort((a,b)=> new Date(a.datetime) - new Date(b.datetime));

    const top = el('section', { class:'card hero', style:'margin-bottom:16px' });
    const header = el('div', { style:'display:flex;justify-content:space-between;align-items:flex-end;gap:12px;flex-wrap:wrap' });
    header.append(el('h2', { style:'font-size:1.6rem;margin:0' }, title));
    top.append(header);

    if(listAll.length){
      const label = includeTasks==='on' ? 'Einträge' : 'Termine';
      top.append(el('div', { class:'meta' }, `Du hast ${listAll.length} ${label} heute:`));
      ...
      if(mode==='detailed'){
        ...
        listAll.forEach(a=>{
          ...
          if(catImages[a.category]) left.append(el('img',{src:catImages[a.category], alt:a.category, style:'width:28px;height:28px;border-radius:6px;object-fit:cover'}));
          const t=new Date(a.datetime).toLocaleTimeString('de-CH',{hour:'2-digit',minute:'2-digit'});
          const titleTxt=(a.title||'').trim()||'(ohne Titel)';
          left.append(el('div',{}, (a.type==='Aufgabe'?'📝 ':'') + titleTxt));
          row.append(left);
        ...
        top.append(el('p', { class:'meta' }, includeTasks==='on'
          ? 'Heute stehen Termine/Aufgaben an.'
          : 'Heute stehen Termine an.'));
      }
    } else {
  ...
  tasks.forEach(a=>{
    ...
    row.append(b1,b2,b3);
    if(a.type!=='Aufgabe'){ const b4=el('button',{type:'button'},'🧾 Bestätigen'); row.append(b4); }
...
function listView(){
  autoUpdate();
  // Prefs
  let next2='on', toggle='off';
  try{
    next2 = localStorage.getItem('tmjw_view_terms_next2') || 'on';
    toggle = localStorage.getItem('tmjw_view_terms_toggle') || 'off';
  }catch(_){}

  // Which submode? (list|cal)
  let mode='list';
  ...
  if(toggle==='on'){
    ...
    btn.onclick = ()=>{
      mode = (mode==='list' ? 'cal' : 'list');
      try{ sessionStorage.setItem('tmjw_terms_mode', mode); }catch(_){}
      listView();
    };
    toolbar.append(btn);
  }
  ...
  if(mode==='cal'){
    ...
    for(let d=1; d<=days; d++){
      ...
      if(isToday) hd.append(el('span', {class:'meta'}, 'heute'));
      cell.append(hd);
      ...
      if(items.length){
        ...
        cell.append(dots);
        cell.onclick = ()=>{
          // open filtered list for that day
          v.innerHTML = '<section><h2>Termine – '+dt.toLocaleDateString('de-CH')+'</h2><div id="list" class="list"></div><div class="btnrow"><button type="button">Zurück</button></div></section>';
          const list=byId('list');
          items.forEach(a=> list.append(renderItem(a, ()=>listView())));
          const back=v.querySelector('button'); back.onclick=()=>listView();
        };
      }
      wrap.append(cell);
    }
  ...
  if(next2==='on'){
    ...
    if(arrTmr.length){
      listWrap.append(el('h3', {}, 'Morgen'));
      arrTmr.forEach(a=> listWrap.append(renderItem(a, ()=>listView())));
      listWrap.append(el('div', {class:'sep'}));
    }
    if(arrD2.length){
      listWrap.append(el('h3', {}, 'Übermorgen'));
  ...
  content.append(listWrap);
    ...
    function contactsView(){
      ...
      <div style="margin-top:24px;opacity:.7;font-size:12px;text-align:center">TimeMate by J.W. Version 1.18</div>
        <h3>Wartung</h3>
  <div class="btnrow">
  ...
  <div style="margin-top:24px;padding-top:12px;border-top:1px dashed var(--border,#444);opacity:.8">
    ...
    function showContactHistory(id, backCat) {
      const c=contacts.find(x=>x.id===id); v.innerHTML=`<section><h2>Verlauf: ${fullName(c)}</h2><div style="margin-top:24px;opacity:.7;font-size:12px;text-align:center">TimeMate by J.W. Version 1.18</div>
      </section>`;
      const s=v.querySelector('section');
      const isMatch = (item) => {
...
function settings(){
  ...
  function showMenu(){
    ...
    btns.append(mk('personal','Personalisierung'));
    btns.append(mk('view','Ansicht'));
    btns.append(mk('io','Export/Import'));
    btns.append(mk('maint','Wartung'));
    content.append(btns);

    const foot = el('div', {style:'margin-top:24px;padding-top:12px;border-top:1px dashed var(--border,#444);opacity:.8'});
    foot.append(el('div', {class:'meta'}, 'TimeMate by J.W. Version 1.21'));
    content.append(foot);
  }
  ...
  function showSection(id){
    ...
    if(id==='view'){
      ...
      box.append(el('h3', {}, 'Ansicht'));

      const grid = el('div', {style:'display:grid;grid-template-columns:240px 1fr;gap:10px'});

      // Heute-Karte
      const labShow = el('label'); labShow.append('Heute-Karte anzeigen');
      const selShow = el('select', {id:'view_today_show'});
      ...
      grid.append(labShow); grid.append(selShow);

      const labMode = el('label'); labMode.append('Heute-Karte • Detailgrad');
      const selMode = el('select', {id:'view_today_mode'});
      ...
      grid.append(labMode); grid.append(selMode);

      const labTasks = el('label'); labTasks.append('Heute-Karte • Aufgaben anzeigen');
      const selTasks = el('select', {id:'view_today_tasks'});
      ...
      grid.append(labTasks); grid.append(selTasks);

      // Termine-Tab Optionen
      const labNext2 = el('label'); labNext2.append('Termine-Tab: „Morgen/Übermorgen“ abtrennen');
      const selNext2 = el('select', {id:'view_terms_next2'});
      ...
      grid.append(labNext2); grid.append(selNext2);

      const labToggle = el('label'); labToggle.append('Termine-Tab: „Ansicht wechseln“-Button');
      const selToggle = el('select', {id:'view_terms_toggle'});
      selToggle.append(el('option',{value:'on'},'An'));
      selToggle.append(el('option',{value:'off'},'Aus'));
      try{ selToggle.value = (localStorage.getItem('tmjw_view_terms_toggle') || 'off'); }catch(_){}
      grid.append(labToggle); grid.append(selToggle);
      ...
      const colorBox = el('div', {class:'card', style:'padding:14px;margin-top:14px'});
      colorBox.append(el('h3', {}, 'Kategorien & Farben'));
      ...
      (CATS_ALL||[]).forEach(c=>{
        ...
        row.append(el('div', {style:'flex:1'}, c.key));
      ...
      colorBox.append(gridC);
      colorBox.append(el('div', {class:'meta', style:'margin-top:6px'}, 'Farben wirken in den Kategorie-Karten & Rändern.'));
      content.append(box); content.append(colorBox); content.append(backBtn());
      return;
    }

    if(id==='personal'){
      const box = el('div', {class:'card', style:'padding:14px'});
      box.append(el('h3', {}, 'Personalisierung'));
      const nameLabel = el('label'); 
      nameLabel.append('Dein Name');
      const nameInp = el('input', {id:'set_name', type:'text', placeholder:'z.B. Joel'});
      try { nameInp.value = localStorage.getItem('tmjw_user_name') || ''; } catch(_){}
      nameLabel.append(nameInp);
      box.append(nameLabel);

      const dmWrap = el('div', {style:'margin-top:8px'});
      const dmLabel = el('label'); dmLabel.append('Dark Mode');
      const dmSel = el('select', {id:'set_theme'});
      ...
      dmLabel.append(dmSel);
      dmWrap.append(dmLabel);
      box.append(dmWrap);
    ...
    if(id==='io'){
      const box = el('div', {class:'card', style:'padding:14px'});
      box.append(el('h3', {}, 'Export / Import'));
      ...
      impWrap.append(inp); impWrap.append(impBtn);
      box.append(impWrap);
      content.append(box);
      content.append(backBtn());
      return;
