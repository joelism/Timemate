(function () {
  ...
  (function(){
    function showErr(msg){
      try{
        ...
        if(!box){
          ...
          box.style.bottom='12px'; box.style.zIndex='99999';
          box.style.padding='12px 14px'; box.style.borderRadius='12px';
          box.style.background='rgba(220,53,69,.12)'; box.style.color='#b00020';
          box.style.backdropFilter='blur(6px)'; box.style.boxShadow='0 4px 16px rgba(0,0,0,.15)';
    ...
    window.addEventListener('error', function(e){
      showErr('JS-Fehler: ' + (e && e.message ? e.message : 'unbekannt'));
    });
    window.addEventListener('unhandledrejection', function(e){
      var msg = (e && e.reason) ? (e.reason.message || e.reason) : 'unbekannt';
      showErr('Promise-Fehler: ' + msg);
    });
  })();
...
<body>
  ...
  <script>setTimeout(function(){ window.print(); }, 250);</script>
    ...
    function form(editId){
      ...
      lT.append(ti); row.append(lT); // Endzeit (optional)
      const lTE=el('label',{class:'half'}); lTE.append('Endzeit (optional)');
      const te=el('input',{id:'timeEnd',type:'time',step:'300'});
      te.addEventListener('change',()=>{
        const [h,m]=(te.value||'').split(':').map(x=>parseInt(x||'0',10));
        if(!isNaN(h)){
          const mm=Math.round((m||0)/5)*5;
          te.value=String(h).padStart(2,'0')+':'+String(mm%60).padStart(2,'0');
        }
      });
      ...
      if(editing){
        ...
        // Prefill optional Endzeit if available
        (function(){
          try{
            if (editing && editing.endDatetime) {
              ...
              if(!isNaN(de.getTime())){
                ...
                const teEl = byId('timeEnd'); if(teEl) teEl.value = `${hh}:${mm}`;
              }
            }
          }catch(_){}
        })();
      ...
      saveBtn.onclick=()=>{
        ...
        const base = { id: editing?editing.id:String(Date.now()), type, title, category:cat, person, location:loc, datetime:dt, notes: byId('notes').value, attachments: tmp, status: editing?editing.status:'upcoming' };st base = { id: editing?editing.id:String(Date.now()), type, title, category:cat, person, location:loc, datetime:dt, notes: byId('notes').value, attachments: tmp, status: editing?editing.status:'upcoming' };
    ...
    function settings(){
      ...
      file.onchange=async()=>{
        ...
        for(const line of lines){
          ...
          const __end = (typeof idx==='function' && idx('Endzeit')>=0) ? (cells[idx('Endzeit')]?.replace(/^\"|\"$/g,'')) : '';
          const dtEnd = (__end && obj.date) ? (function(){ try{ const [d,m,y]=obj.date.split('.'); return new Date(`${String(y).padStart(4,'0')}-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}T${__end}:00`);}catch(_){return null;}})() : null;
          out.push({ id:obj.id, type:obj.type, title:obj.title, category:obj.category,
            ...
            location:obj.location, datetime: dt.toISOString(), notes:obj.notes, status:obj.status, attachments:[], endDatetime: (dtEnd && !isNaN(dtEnd.getTime())) ? dtEnd.toISOString() : '' });
        }
