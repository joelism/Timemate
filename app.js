/* TimeMate by J.W. — app.js (UI-Polish)
   - wie zuvor + kompaktere Terminliste auf Mobile (Buttons in einer Zeile, kleinere Pills)
   - zeigt Endezeit in Liste
   - Kontakte/Einstellungen bleiben
*/
(function () {
  ...
  function injectStyles(){
    const css = `
    .list .row{display:grid;grid-template-columns:1fr auto;gap:12px;padding:10px 0;border-bottom:1px solid #eef2f7}
    .list .row:last-child{border-bottom:0}
    .list .row .meta{color:#6b7280;font-size:.95rem;margin-top:2px}
    .list .row .btnbar{display:flex;gap:8px;overflow:auto;white-space:nowrap}
    .list .row .btnbar > button{font-size:.95rem;padding:.35rem .6rem;border-radius:999px}
  ...
  function init() {
    ...
    const el = (t, attrs = {}, text) => { const n = document.createElement(t); Object.entries(attrs).forEach(([k,v])=>n.setAttribute(k,v)); if(text!==undefined) n.textContent=text; return n; };
    ...
    function listView(){ const items=state.items.filter(i=>i.type!=='Aufgabe' && i.status!=='archived'); view.innerHTML=`<section class="panel"><h2>Termine</h2>${renderItems(items)||'<p class="meta">Nichts vorhanden.</p>'}</section>`; }
    function tasksView(){ const items=state.items.filter(i=>i.type==='Aufgabe' && i.status!=='archived'); view.innerHTML=`<section class="panel"><h2>Aufgaben</h2>${renderItems(items)||'<p class="meta">Nichts vorhanden.</p>'}</section>`; }
    function archiveView(){ const items=state.items.filter(i=>i.status==='archived'); view.innerHTML=`<section class="panel"><h2>Archiv</h2>${renderItems(items)||'<p class="meta">Nichts vorhanden.</p>'}</section>`; }
    ...
    function settingsView(){
      ...
      const addrInp=byId('catAddrInput'); const fill=()=> addrInp.value = catAddr[sel.value]||''; sel.addEventListener('change',fill); fill();
      byId('saveCatAddr').addEventListener('click',()=>{ catAddr[sel.value]=(addrInp.value||'').trim(); saveCatAddr(); alert('Hauptadresse gespeichert.'); });
    }
    ...
    function contactsView(){
      ...
      const draw=()=>{
        ...
        contacts.forEach(c=>{
          ...
          const right=el('div',{class:'btnbar'});
          const editB=el('button',{},'Bearbeiten'); const delB=el('button',{},'Löschen');
          editB.onclick=()=>editContact(c);
          delB.onclick=()=>{ if(confirm('Kontakt löschen?')){ contacts=contacts.filter(x=>x!==c); saveContacts(); draw(); } };
          right.append(editB,delB); row.append(right); clist.append(row);
        });
    ...
    function editor(editing){
      ...
      function fillPersons(){ selPer.innerHTML=''; const persons=personsForCategory(selCat.value); ['Andere',...persons].forEach(p=> selPer.append(el('option',{},p))); }
      function fillLocationPicker(){
        selLoc.innerHTML=''; const opts=[]; const h=catAddr[selCat.value]; if(h) opts.push({v:'__haupt__',t:`Hauptadresse (${h})`});
        const addrSet=new Set(); contacts.filter(c=>c.kategorie===selCat.value && c.adresse).forEach(c=>addrSet.add(c.adresse)); [...addrSet].forEach(a=>opts.push({v:`c:${a}`,t:a}));
        opts.push({v:'__other__',t:'Andere…'}); opts.forEach(o=> selLoc.append(el('option',{value:o.v},o.t)));
      }
      ...
      selLoc.addEventListener('change',syncLocInput); fillPersons(); fillLocationPicker(); syncLocInput();
      if(editing){ byId('t_title').value=editing.title||''; selCat.value=editing.category||selCat.value; fillPersons();
        const personVal=Array.isArray(editing.person)?editing.person[0]:(editing.person||'Andere'); selPer.value=personsForCategory(selCat.value).includes(personVal)?personVal:'Andere';
        const d=new Date(editing.datetime); byId('t_date').value=d.toISOString().slice(0,10); byId('t_time').value=`${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
        if(editing.endtime) byId('t_endtime').value=editing.endtime; inpLoc.value=editing.location||''; byId('t_notes').value=editing.notes||''; }
      byId('cancelT').onclick=()=>route('overview');
      byId('saveT').onclick=()=>{ const title=byId('t_title').value.trim(); const cat=selCat.value||CAT_UNCAT; const date=byId('t_date').value; const time=byId('t_time').value;
        if(!title||!date||!time){ alert('Bitte Titel, Kategorie, Datum und Uhrzeit angeben.'); return; }
        const person=selPer.value==='Andere'?'Andere':selPer.value; const dt=new Date(`${date}T${time}:00`).toISOString();
        const obj={ id: editing?editing.id:String(Date.now()), type:'Termin', title, category:cat, person, datetime:dt, endtime:byId('t_endtime').value||'', location:(inpLoc.value||'').trim(), notes:byId('t_notes').value.trim(), attachments:[], status: editing?editing.status:'upcoming' };
    ...
    function renderItems(items){
      ...
      sorted.forEach(it=>{
        ...
        const metaLine=[ fmtDate(it.datetime) + (it.endtime? '–'+it.endtime : ''), it.person||'', it.location||'' ].filter(Boolean).join(' · ');
        left.append(el('div',{class:'meta'}, metaLine));
        row.append(left);
        const right=el('div',{class:'btnbar'});
        ...
        right.append(bE,bD,bA,bC); row.append(right); wrap.append(row);
      });
    ...
    function openConfirmDoc(item){
      try{
        ...
        const perDisp=Array.isArray(item.person)?item.person.join(', '):(item.person||''); const nowStr=new Date().toLocaleString('de-CH',{dateStyle:'short',timeStyle:'short'});
        const statusLabel=STATUS_LABEL[item.status]||item.status; const title=item.title||'Termin';
        const css=`body{font-family:system-ui,-apple-system,Segoe UI,Roboto;background:#f7fafc;color:#111827;padding:28px;}
        h1{margin:0 0 6px}.sub{color:#6b7280;margin:0 0 16px}.box{border:2px solid #1e90ff;border-radius:12px;background:#fff;padding:16px}
        .row{display:grid;grid-template-columns:140px 1fr;gap:10px;padding:6px 0;border-bottom:1px solid #eef2f7}.row:last-child{border:0}
        .label{color:#6b7280}.badge{display:inline-block;border:1px solid #e5e7eb;border-radius:999px;padding:.1rem .5rem;font-size:.9rem}
