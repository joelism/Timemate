...
/* Safeguard: ensure main address defaults on submit if empty */
(function(){
  if (window.__TM_ADDR_ENHANCER_BASE__) return; window.__TM_ADDR_ENHANCER_BASE__=true;
  let catAddr = {};
  try{ catAddr = JSON.parse(localStorage.getItem('tmjw_cat_addr')||'{}'); }catch{ catAddr = {}; }
  document.addEventListener('submit', function(ev){
    try{
      const form = ev.target;
      const catSel = form.querySelector('select[name="category"], #category, select#category');
      const loc = form.querySelector('input[name="location"], #location');
      if (!catSel || !loc) return;
      const main = (catAddr && catAddr[catSel.value]) ? String(catAddr[catSel.value]).trim() : '';
