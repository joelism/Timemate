...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form && !form.querySelector('#datetimeEnd')) {
    const start = form.querySelector('#datetime') || form.querySelector('input[type="datetime-local"]');
    if (start) {
      const label = document.createElement('label');
      label.textContent = 'Ende (optional)';
      const end = document.createElement('input');
      end.type = 'datetime-local';
      end.id = 'datetimeEnd';
      end.name = 'datetimeEnd';
...
// --- Standort automatisch auf Hauptadresse setzen, falls leer ---
window.addEventListener('submit', e => {
  const form = e.target;
  if (!form.querySelector || !window.catAddr) return;
  const cat = form.querySelector('select[name="category"]');
  const loc = form.querySelector('input[name="location"]');
  if (cat && loc && !loc.value && catAddr[cat.value]) {
    loc.value = catAddr[cat.value];
  }
}, true);
