(function () {
  ...
  function generateConfirmationPDF(entry){
    try{
      const {date, time} = formatSlash(entry.datetime);
      const participants = uniq(['Joel Weber', ...ensureArray(entry.person)]).join(', ');

      const w = window.open('', '_blank', 'noopener,noreferrer');
      if(!w){ alert('Popup blockiert – bitte Popups für diese Seite erlauben.'); return; }

      const logo = 'icons/icon-180x180.png'; // App-Logo
      const title = 'Terminbestätigung';
      const name = entry.title || '(ohne Titel)';
      const footer = 'Diese Terminbestätigung wurde automatisch von TimeMate by J.W. erstellt.';
      ...
      w.document.write(`<!doctype html><html><head><meta charset="utf-8">${styles}</head><body>
        <div class="head">
          <img src="${logo}" alt="TimeMate Logo">
          <div class="brand">TimeMate by J.W.</div>
        </div>
        <h1>${title}</h1>
      ...
      const tryPrint = () => {
        ...
        let loaded = 0;
        const done = () => { setTimeout(() => { w.focus(); w.print(); }, 150); };
        if(!imgs || imgs.length===0){ done(); return; }
  ...
  function form(editId){
    ...
    // >>> Beim Öffnen eines bestehenden Termins sofort PDF generieren
    if (editing && (editing.type!=='Aufgabe')) {
      // Nur für Termine, nicht für Aufgaben
      generateConfirmationPDF(editing);
    }
