...
// Notizen + Anhänge
      ...
      function populateCats(){
        selCat.innerHTML='';
        CATS_ALL.forEach(c=>selCat.append(el('option',{}
      // === Hauptadresse der Kategorie automatisch in Standort übernehmen ===
      function applyCatMainAddress() {
        try{
          const MAIN = catAddr || {};
          const cat = selCat && selCat.value;
          const addr = (MAIN && cat) ? MAIN[cat] : '';
          // find location element (may be input or select, recreated by fillDyn)
          const locEl = byId('location') || document.querySelector('#dyn #location');
          if(!locEl || !addr) return;
          if(locEl.tagName === 'INPUT'){
            // Input: wenn nicht manuell verändert ODER leer → setzen
            if(!locTouched || !locEl.value || !locEl.value.trim()){
              locEl.value = addr;
            }
            locEl.oninput = ()=>{ locTouched=true; };
          }else if(locEl.tagName === 'SELECT'){
            // SELECT: Option sicherstellen
            let exists = Array.from(locEl.options).some(o => (o.value||o.textContent) === addr);
            if(!exists){
              const opt = document.createElement('option');
              opt.textContent = addr; opt.value = addr;
              locEl.insertBefore(opt, locEl.firstChild);
            }
            // Wenn der Nutzer den Standort noch nicht manuell geändert hat, setzen wir die Hauptadresse
            if(!locTouched){
              locEl.value = addr;
            }
            locEl.onchange = ()=>{ locTouched=true; };
          }
        }catch(_){}
      }
,c.key)));
      ...
      selCat.addEventListener('change',()=>{ fillDyn(selType.value, selCat.value, dyn);
        applyCatMainAddress(); applyCatMainAddress(); });
      populateCats(); applyCatMainAddress();
