...
// Notizen + Anhänge
      ...
      // === Hauptadresse der Kategorie automatisch in Standort übernehmen (wenn leer) ===
      function applyCatMainAddress() {
        try{
          const MAIN = catAddr || {};
          const cat = selCat && selCat.value;
          if(!cat || !MAIN[cat]) return;
          // location element may be <input id="location"> or <select id="location">
          const locEl = byId('location') || document.querySelector('#dyn #location');
          if(!locEl) return;
          // Don't overwrite if user already typed something / value chosen
          if(locEl.tagName === 'INPUT') {
            if(!locEl.value || !locEl.value.trim()) locEl.value = MAIN[cat];
          } else if (locEl.tagName === 'SELECT') {
            // Add option if missing
            const exists = Array.from(locEl.options).some(o => (o.value||o.textContent) === MAIN[cat]);
            if(!exists) {
              const opt = document.createElement('option');
              opt.textContent = MAIN[cat];
              opt.value = MAIN[cat];
              locEl.insertBefore(opt, locEl.firstChild);
            }
            // Select it only if empty/unset
            if(!locEl.value || !locEl.value.trim()) locEl.value = MAIN[cat];
          }
        }catch(_){}
      }
