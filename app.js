...
function openConfirmDoc(item){
  try{
    ...
    const dte = new Date(item.datetimeEnd||'');
    const timeStrEnd = !isNaN(dte) ? dte.toLocaleTimeString('de-CH',{hour:'2-digit',minute:'2-digit'}) : '';
    const rangeStr = (timeStr && timeStrEnd) ? (timeStr+' – '+timeStrEnd) : timeStr;
  ...
  <script>setTimeout(function(){ window.print(); }, 250);</script>
    ...
    function form(editId){
      ...
      saveBtn.onclick=()=>{
        ...
        const locRaw = byId('location') ? byId('location').value : '';
        const mainAddr = (catAddr && catAddr[cat]) ? String(catAddr[cat]).trim() : '';
        const locFinal = (locRaw && locRaw.trim()) ? locRaw.trim() : mainAddr;
        const dt  = new Date(`${date}T${time}:00`).toISOString();
        const timeEnd = byId('timeEnd') ? byId('timeEnd').value : '';
        const dtEnd = timeEnd ? new Date(`${date}T${timeEnd}:00`).toISOString() : '';
        const base = { id: editing?editing.id:String(Date.now()), type, title, category:cat, person, location:loc, datetime: dt,
          datetimeEnd: dtEnd,
          dt, notes: byId('notes').value, attachments: tmp, status: editing?editing.status:'upcoming' };
