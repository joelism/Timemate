...
function settings(){
  ...
  function showMenu(){
    ...
    foot.append(el('div', {class:'meta'}, 'TimeMate by J.W. Version 1.20'));
    content.append(foot);
  }
  ...
  function showSection(id){
    ...
    if(id==='view'){
      ...
      const t1 = el('div', {style:'display:grid;grid-template-columns:220px 1fr;gap:10px'});
      ...
      t1.append(labShow); t1.append(selShow);

      const labMode = el('label'); labMode.append('Detailgrad');
      ...
      t1.append(labMode); t1.append(selMode);

      const labTasks = el('label'); labTasks.append('Aufgaben des Tages anzeigen');
      ...
      t1.append(labTasks); t1.append(selTasks);
      ...
      saveV.onclick = ()=>{
        try{
          ...
          alert('Ansicht gespeichert. Öffne „Übersicht“, um das Ergebnis zu sehen.');
      ...
      const grid = el('div', {style:'display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px'});
      (CATS_ALL||[]).forEach(c=>{
        ...
        input.oninput = ()=>{
          const m = __loadColors();
          m[__norm(c.key)] = input.value;
          __saveColors(m);
        };
        row.append(input);
        grid.append(row);
      });
      colorBox.append(grid);
      const hint = el('div', {class:'meta', style:'margin-top:6px'}, 'Tipp: Farben wirken in den Kategorie-Karten und Rändern.'); 
      colorBox.append(hint);
