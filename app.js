(function () {
  ...
  // === Kategorie-Farben (Übersicht) mit festen Zuordnungen + Auto-Palette ===
  function __norm(str){ 
  ...
  function __saveColors(map){ try{ localStorage.setItem(__CAT_COLOR_KEY, JSON.stringify(map)); } catch(_){} }
  function __seedDefaults(colors){
    const defaults = {
    ...
    let changed=false;
    for(const [k,v] of Object.entries(defaults)){ if(!colors[k]){ colors[k]=v; changed=true; } }
    if(changed) __saveColors(colors);
    return colors;
  }
  function __ensureColor(cat){
    const key = __norm(cat); if(!key) return null;
    let colors = __seedDefaults(__loadColors());
    ...
    colors[key]=free; __saveColors(colors); return free;
  }
  function catColor(cat){ return __ensureColor(cat); }
  function catBadge(cat){
    if(!cat) return '';
    const col = catColor(cat), bg = col || '#E0E0E0', fg = col ? '#fff' : '#333';
    const text = String(cat).replace(/</g,'&lt;').replace(/>/g,'&gt;');
    return `<span class="cat-badge" style="display:inline-block;padding:2px 8px;border-radius:9999px;font-size:12px;font-weight:600;margin-right:8px;background:${bg};color:${fg};vertical-align:middle;">${text}</span>`;
  }
...
function openConfirmDoc(item){
  try{
    ...
    const endValid = item.endDatetime ? !isNaN(new Date(item.endDatetime).getTime()) : false;
    const endStr = endValid ? new Date(item.endDatetime).toLocaleTimeString('de-CH', { hour:'2-digit', minute:'2-digit' }) : '';
...
<body>
  ...
  <div class="box">
    ...
    <div class="row"><div class="label">Datum</div><div>${esc(dateStr)}${timeStr ? ' – ' + esc(timeStr) : ''}${endStr ? ' – ' + esc(endStr) : ''} Uhr</div></div>
  ...
  <script>setTimeout(function(){ window.print(); }, 250);</script>
    ...
    function form(editId){
      ...
      // === Auto-Standort: zuerst Personen-Adresse (Kontakt.adresse), sonst Kategorie-Hauptadresse (catAddr) ===
      ...
      function __getSelectedPersons(){
        const m = byId('personMulti'); const s = byId('person');
        if (m) return Array.from(m.selectedOptions).map(o=>o.value);
        if (s) return [s.value];
        const other = byId('personOther');
      ...
      function applyAutoLocation(){
        try{
          ...
          // 2) Fallback: Kategorie-Hauptadresse
          ...
          if(addr2){
            if(locEl.tagName==='INPUT'){ if(empty) locEl.value = addr2; }
              ...
              if(!ok){ const op2=document.createElement('option'); op2.value=addr2; op2.textContent=addr2; locEl.insertBefore(op2, locEl.firstChild); }
      ...
      function hookAutoLocationListeners(){
        try{
          const locEl = byId('location') || document.querySelector('#dyn #location');
          if(locEl && !locEl.__bound){
            if(locEl.tagName==='INPUT') locEl.addEventListener('input', ()=>{ __locTouched=true; });
            else locEl.addEventListener('change', ()=>{ __locTouched=true; });
            locEl.__bound = true;
          }
          const selM = byId('personMulti'); const sel = byId('person'); const other=byId('personOther');
          [selM, sel].forEach(x=>{ if(x && !x.__boundAuto){ x.addEventListener('change', applyAutoLocation); x.__boundAuto=true; }});
          if(other && !other.__boundAuto){ other.addEventListener('input', applyAutoLocation); other.__boundAuto=true; }
        }catch(_){}
      }
      ...
      lT.append(ti); row.append(lT); const lTE=el('label',{class:'half'}); lTE.append('Endzeit (optional)'); const te=el('input',{id:'timeEnd',type:'time',step:'300'});
te.addEventListener('change',()=>{ const [h,m]=(te.value||'').split(':').map(x=>parseInt(x||'0',10)); if(!isNaN(h)){ const mm=Math.round((m||0)/5)*5; te.value=String(h).padStart(2,'0')+':'+String(mm%60).padStart(2,'0'); }});
lTE.append(te); row.append(lTE);
s.append(row);
// Notizen + Anhänge
      ...
      selCat.addEventListener('change',()=>{ fillDyn(selType.value, selCat.value, dyn);
        applyAutoLocation(); hookAutoLocationListeners(); applyAutoLocation(); hookAutoLocationListeners(); });
      ...
      if(editing){
        ...
        if (editing && editing.endDatetime) {
          const de=new Date(editing.endDatetime);
          if(!isNaN(de.getTime())){
            const hh=String(de.getHours()).padStart(2,'0');
            const mm=String(de.getMinutes()).padStart(2,'0');
            const teEl=document.getElementById('timeEnd'); if(teEl) teEl.value=`${hh}:${mm}`;
          }
        }
      ...
      saveBtn.onclick=()=>{
        ...
        const timeEndVal = (document.getElementById('timeEnd') && document.getElementById('timeEnd').value) ? document.getElementById('timeEnd').value : '';
        const dtEnd = (timeEndVal && date) ? new Date(`${date}T${timeEndVal}:00`).toISOString() : '';
const base = { id: editing?editing.id:String(Date.now()), type, title, category:cat, person, location:loc, datetime:dt, notes: byId('notes').value, attachments: tmp, status: editing?editing.status:'upcoming', endDatetime: dtEnd };
    ...
    function exportCSV(items){
      ...
      items.slice().sort((a,b)=>new Date(a.datetime)-new Date(b.datetime)).forEach(a=>{
        const d=new Date(a.datetime), date=d.toLocaleDateString('de-CH'), time=d.toLocaleTimeString('de-CH',{hour:'2-digit',minute:'2-digit'}); const te = a.endDatetime ? new Date(a.endDatetime).toLocaleTimeString('de-CH',{hour:'2-digit',minute:'2-digit'}) : '';
      ...
      file.onchange=async()=>{
        ...
        for(const line of lines){
          ...
          const obj={
            ...
            end: idx('Endzeit')>=0 ? (cells[idx('Endzeit')]?.replace(/^"|"$/g,'')) : '',
          ...
          out.push({ id:obj.id, type:obj.type, title:obj.title, category:obj.category,
            ...
            location:obj.location, datetime: dt.toISOString(), endDatetime: (function(){ if(!obj.end) return ''; try{ const [d2,m2,y2]=obj.date.split('.'); const e=new Date(`${y2}-${m2.padStart(2,'0')}-${d2.padStart(2,'0')}T${obj.end}:00`); return isNaN(e.getTime()) ? '' : e.toISOString(); }catch(_){ return ''; } })(), notes:obj.notes, status:obj.status, attachments:[] });
        }
