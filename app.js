...
function contactsView(){
  ...
  const resultsSec = el('div', {}); v.querySelector('section').append(resultsSec);
  const renderSearch = ()=>{
    const q = (searchInput.value||'').trim().toLowerCase();
    resultsSec.innerHTML = '<h3>Suchergebnisse</h3>';
    if(!q){ resultsSec.append(el('p',{class:'meta'},'Gib etwas ein, um zu suchen…')); return; }
    const hits = contacts.filter(c=>{
      const hay = [fullName(c), c.kategorie||'', c.funktion||'', c.telefon||'', c.email||'', c.notizen||''].join(' ').toLowerCase();
      return hay.includes(q);
    });
    if(!hits.length){ resultsSec.append(el('p',{class:'meta'},'Keine Treffer.')); return; }
    const grid = el('div', {style:'display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px'});
    hits.forEach(c=> grid.append(contactCard(c)));
    resultsSec.append(grid);
  };
...
function settings(){
  ...
  function showMenu(){
    ...
    foot.append(el('div', {class:'meta'}, 'TimeMate by J.W. Version 1.23'));
    content.append(foot);
  }
  ...
  function groupCard(title, subtitle){
    ...
    if(subtitle) card.append(el('div', {class:'meta', style:'margin:-6px 0 10px;opacity:.85'}, subtitle));
    return card;
  }
  ...
  function rowWrap(){
    ...
    if(compact){
      // stacked
      return el('div', {style:'display:flex;flex-direction:column;gap:4px;margin:8px 0'});
    }else{
      return el('div', {style:'display:grid;grid-template-columns:260px 1fr;gap:10px;align-items:center;margin:6px 0'});
    }
  }
  ...
  function showSection(id){
    ...
    if(id==='view'){
      // ==== Gruppe: Heute-Karte ====
      const cardToday = groupCard('Ansicht • Heute-Karte', 'Steuert die Karte oben im Tab „Übersicht“.');
      // show toggle
      const r1 = rowWrap();
      r1.append(el('label',{},'Heute-Karte anzeigen'));
      ...
      // include tasks
      const r3 = rowWrap();
      r3.append(el('label',{},'Aufgaben des Tages anzeigen'));
      ...
      // ==== Gruppe: Termine-Tab ====
      const cardTerms = groupCard('Ansicht • Termine-Tab', 'Listen-/Kalenderdarstellung und Hervorhebung.');
      const r4 = rowWrap();
      r4.append(el('label',{},'„Morgen/Übermorgen“ abtrennen'));
      ...
      const r5 = rowWrap();
      r5.append(el('label',{},'„Ansicht wechseln“-Button'));
      ...
      const cardColors = groupCard('Kategorien & Farben', 'Farbpunkte in Karten & Kalender.');
      ...
      content.append(cardToday);
      content.append(cardTerms);
      content.append(cardColors);
      content.append(saveV);
    ...
    if(id==='personal'){
      ...
      const dmWrap = rowWrap();
      dmWrap.append(el('label', {}, 'Dark Mode'));
