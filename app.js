(function () {
  ...
  // ====== Darkmode Styles und Tab-Scrollbar ======
  ...
  function openConfirmationPDF(entry){
    try {
      ...
      <style>
        ...
        <div class="head">
          <img class="logo" src="icons/icon-180x180.png" alt="Logo">
    ...
    } catch(e){ console.error('PDF-Fehler', e); alert('PDF konnte nicht erstellt werden'); }
  }
    ...
    if(!nav.querySelector('[data-route="tasks"]')){
      const t=document.createElement('button'); t.className='tab'; t.dataset.route='tasks'; t.textContent='Aufgaben'; t.onclick=()=>route('tasks'); nav.appendChild(t);
    }
...
if(a.type!=='Aufgabe'){const b4=el('button',{type:'button'},'🧾 Bestätigung'); b4.onclick=()=>openConfirmationPDF(a); row.append(b4);}
        row.append(b1,b2,b3,b4); it.append(row); list.append(it);
      });
      wrap.append(list);
      v.append(wrap);
    }
  ...
  const pdfBtn=document.createElement('button'); pdfBtn.textContent='🧾 Terminbestätigung (PDF)'; pdfBtn.onclick=()=>openConfirmationPDF(editing); s.append(pdfBtn); setTimeout(()=>openConfirmationPDF(editing),200);
}
  v.append(s);
