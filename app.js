...
<head>
<meta charset="utf-8">
<title>Terminbestätigung – ${esc(title)}</title>
<style>
  @page { margin: 18mm; }
  ...
  <script>setTimeout(function(){ window.print(); }, 250);</script>
    ...
    function ov(){
      ...
      CATS_ALL.forEach(c=>{
        ...
        if(next){
          const pArr = Array.isArray(next.person) ? next.person.slice() : (next.person ? [next.person] : []);
          if(!pArr.some(x => (x||'').trim().toLowerCase()==='joel weber')) pArr.push('Joel Weber');
          const p = pArr.length ? pArr.join(', ') : '—';
          ...
          // Bestätigen-Button
          try{ const b4=el('button',{type:'button'},'🧾 Bestätigen'); b4.onclick=()=>openConfirmDoc(next); row.append(b4); }catch(_e){}
    ...
    function renderItem(a, refresh){
      const it=el('div',{class:'item'});
      const pplArr = Array.isArray(a.person) ? a.person.slice() : (a.person ? [a.person] : []);
      if(!pplArr.some(x => (x||'').trim().toLowerCase()==='joel weber')) pplArr.push('Joel Weber');
      const pDisp = pplArr.length ? pplArr.join(', ') : '—';
