(function () {
  ...
  // === Kategorie-Farben (Übersicht) mit fester Zuordnung + Auto-Farben für neue Kategorien ===
  function __norm(str){ 
  ...
  function __loadColors(){
    try{ return JSON.parse(localStorage.getItem(__CAT_COLOR_KEY) || '{}') || {}; } catch(_){ return {}; }
  }
  function __saveColors(map){
    try{ localStorage.setItem(__CAT_COLOR_KEY, JSON.stringify(map)); } catch(_){}
  }

  // Seed mit festen Farben für deine aktuellen 7 Kategorien
  function __seedDefaults(colors){
    ...
    for(const [k,v] of Object.entries(defaults)){
      if(!colors[k]){ colors[k]=v; changed=true; }
    }
  ...
  function __ensureColor(cat){
    const key = __norm(cat);
    if(!key) return null;
    let colors = __loadColors();
    colors = __seedDefaults(colors);
    if(colors[key]) return colors[key];
    // Nächste freie Farbe aus Palette wählen (keine Duplikate bevorzugen)
