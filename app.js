/* TimeMate by J.W. — app.js (Layout wie früher)
   - Keine zusätzlichen Style-Injektionen, Buttons/Abstände wie zuvor
   - Ende (optional), Standort-Auswahl (Hauptadresse/Kontakte/Andere…),
     Kontakte & Einstellungen, Bestätigen-Dokument (blauer Rahmen + Branding)
   - Verwendet weiter deine bestehenden LocalStorage-Keys:
     tmjw_state, tmjw_contacts, tmjw_cat_addr, tmjw_cats_all
*/
(function () {
  ...
  function init() {
    ...
    function contactsView(){
      ...
      const draw=()=>{
        ...
        contacts.forEach(c=>{
          ...
          const right=el('div',{class:'btnrow'});
    ...
    function openConfirmDoc(item){
      try{
        ...
        const perDisp=Array.isArray(item.person)?item.person.join(', '):(item.person||'Joel Weber');
        if(!perDisp.toLowerCase().includes('joel weber')){ /* sicherstellen */ }
