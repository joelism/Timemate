...
<body>
  ...
  <script>setTimeout(function(){ window.print(); }, 250);</script>
    ...
    // ====== Übersicht ======
    function ov(){
      autoUpdate(); v.innerHTML='';// === Begrüssung & Heutige Termine (themenfreundlicher Card-Stil) ===
(function(){
...
function listView(){
  ...
  if(mode==='cal'){
    ...
    let maxRange = 12;
    try{ maxRange = parseInt(localStorage.getItem('tmjw_view_terms_calrange')||'12',10); }catch(_){}
    let vy, vm;
    try{
      const s = sessionStorage.getItem('tmjw_cal_cursor');
      if(s){ const o = JSON.parse(s); vy=o.y; vm=o.m; }
    }catch(_){}
    if(vy==null || vm==null){ vy=today.getFullYear(); vm=today.getMonth(); }
    function saveCursor(){ try{ sessionStorage.setItem('tmjw_cal_cursor', JSON.stringify({y:vy,m:vm})); }catch(_){} }
    ...
    function withinForwardLimit(y,m){
      const diff = (y - today.getFullYear())*12 + (m - today.getMonth());
      return diff >= 0 && diff <= (maxRange-1);
    }
    function withinBackwardLimit(y,m){
      const diff = (y - today.getFullYear())*12 + (m - today.getMonth());
      return diff <= 0;
    }
    ...
    left.onclick = ()=>{
      ...
      if(withinBackwardLimit(ny,nm)){ vy=ny; vm=nm; saveCursor(); listView(); }
    };
    right.onclick = ()=>{
...
function settings(){
  ...
  function showSection(id){
    ...
    if(id==='view'){
      ...
      const r6 = rowWrap();
      r6.append(el('label',{},'Kalender: Reichweite'));
