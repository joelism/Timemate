...
/* ====== Override: Terminbestätigung (Dokument) – robust, mit Dateinamen & DE-Labels ====== */
function openConfirmDoc(item){
  try{
    // Helpers
    ...
    function sanitizeForFilename(s){ return String(s||'').replace(/[^A-Za-z0-9_. -]/g,'').replace(/\s+/g,'_').slice(0,80) || 'ohne_Titel'; }
    ...
    // Dateiname (für PDF-Dialog)
    const datePart  = valid ? yyyymmdd(dt) : yyyymmdd(new Date());
    const titlePart = sanitizeForFilename(item.title || '(ohne Titel)');
    ...
    const html =
      ...
      '<div class="box">' +
        ...
        '<div class="row"><div class="label">Status</div><div>'+ esc(statusLabel) +'</div></div>' +
        '<div class="row"><div class="label">Notizen</div><div>'+ esc(item.notes || '—') +'</div></div>' +
        '<div class="row"><div class="label">Anhänge</div><div>'+ esc((item.attachments&&item.attachments.length)?item.attachments.map(a=>a.name||'Anhang').join(', '):'—') +'</div></div>' +
        '<div class="row small"><div class="label">ID</div><div>'+ esc(item.id || '') +'</div></div>' +
      '</div>' +
    ...
    // Nur Print-Tab öffnen (stabil auf iOS/Safari/Chrome)
    const ww = window.open('', '_blank');
    if(!ww){ alert('Popup blockiert – bitte Popups erlauben.'); return; }
    ww.document.open('text/html');
    ww.document.write(html);
