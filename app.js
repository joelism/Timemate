...
(function(){
  ...
  if(todayItems.length){
    ...
    function contactsView(){
      ...
      <div style="margin-top:24px;opacity:.7;font-size:12px;text-align:center">TimeMate by J.W. Version 1.19</div>
        <h3>Wartung</h3>
  <div class="btnrow">
  ...
  <div style="margin-top:24px;padding-top:12px;border-top:1px dashed var(--border,#444);opacity:.8">
    <div class="meta">TimeMate by J.W. Version 1.19</div>
  </div>
</section>`;
    ...
    function showContactHistory(id, backCat) {
      const c=contacts.find(x=>x.id===id); v.innerHTML=`<section><h2>Verlauf: ${fullName(c)}</h2><div style="margin-top:24px;opacity:.7;font-size:12px;text-align:center">TimeMate by J.W. Version 1.19</div>
      </section>`;
      const s=v.querySelector('section');
      const isMatch = (item) => {
...
function settings(){
  ...
  // Subtab bar
  const tabs = el('div', {class:'tabs', style:'display:flex;gap:8px;flex-wrap:wrap;margin:8px 0 16px'});
  const makeTab = (id, label) => {
    const b = el('button', {class:'tab', 'data-sub':id, type:'button', style:'padding:8px 12px;border-radius:10px;border:1px solid var(--border,#444)'} , label);
    b.onclick = ()=> showTab(id);
    return b;
  };
  tabs.append(makeTab('personal', 'Personalisierung'));
  tabs.append(makeTab('io',        'Export/Import'));
  tabs.append(makeTab('maint',     'Wartung'));
  s.append(tabs);
  ...
  function setActive(id){
    [...tabs.querySelectorAll('button.tab')].forEach(b=>b.classList.toggle('active', b.dataset.sub===id));
  }
  ...
  function showTab(id){
    ...
    if(id === 'personal'){
      ...
      // Darkmode
      const tRow = el('div', {style:'margin-top:8px'});
      ...
      try{
        const cur = localStorage.getItem('tmjw_theme') || 'light';
        dmSel.value = cur;
      }catch(_){}
      ...
      const p = el('p', {class:'meta'}, 'Sichere oder lade deine Daten (Termine, Aufgaben, Kontakte, Kategorien).');
      box.append(p);
      ...
      impBtn.onclick = async ()=>{
        if(!impInput.files || !impInput.files[0]) { alert('Bitte Datei auswählen.'); return; }
        try{
          const txt = await impInput.files[0].text();
        ...
        }catch(e){ console.error(e); alert('Import fehlgeschlagen. Datei gültig?'); }
      };
      impWrap.append(impInput); impWrap.append(impBtn);
      box.append(impWrap);
      content.append(box);
      ...
      const txt = el('p', {class:'meta'}, 'Werkzeuge zum Aufräumen und Verwalten.');
      box.append(txt);
      const row = el('div', {class:'btnrow'});
    ...
    foot.append(el('div', {class:'meta'}, 'TimeMate by J.W. Version 1.19'));
    content.append(foot);
  }
...
const _saveNameBtn = byId('save-username'); if(_saveNameBtn){ _saveNameBtn.onclick=()=>{ 
  const val = (byId('set-username')?.value||'').trim();
  try{ localStorage.setItem('tmjw_user_name', val); alert('Name gespeichert.'); }catch(_){ alert('Konnte den Namen nicht speichern.'); }
}; }

// Wartung: Archiv öffnen & Alle Termine löschen
const _archBtn = byId('open-arch'); if(_archBtn) _archBtn.onclick=()=>arch();
const _delAll  = byId('del-all');  if(_delAll) _delAll.onclick=()=>{
